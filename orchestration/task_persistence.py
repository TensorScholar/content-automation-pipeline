...
class TaskResultRepository:
    ...
    async def update_task_retry(
        ...
        """
        Mark task as RETRY without promoting it to terminal failure.
        """
        from sqlalchemy import update
...
class SyncTaskResultRepository:
    """
    Synchronous repository for task persistence in Celery workers.

    Uses synchronous database operations to avoid async event loop conflicts
    in Celery's multiprocessing environment.
    """
    ...
    def __init__(self, sync_db=None):
        ...
        Args:
            sync_db: Optional SyncDatabaseManager instance. If None, uses global instance.
        """
        if sync_db is None:
    ...
    def create_task_record(
        ...
        Returns:
            Database record UUID as string
        """
        import json
        ...
        # Existing row (e.g. retry execution path) — return current ID.
        existing = self.db.execute(
            "SELECT id FROM task_results WHERE task_id = %(task_id)s",
            {"task_id": task_id},
            fetch_one=True,
        )
    ...
    def update_task_success(
        ...
        """
        Update task record on successful completion (synchronous).
    ...
    def update_task_retry(
        ...
        query = """
            ...
            SET status = %(status)s,
                error = %(error)s,
                traceback = %(traceback)s,
                retry_count = GREATEST(COALESCE(retry_count, 0), %(retry_count)s),
                updated_at = %(updated_at)s
            WHERE task_id = %(task_id)s
        """
        params = {
    ...
    def get_task_by_id(self, task_id: str) -> Optional[Dict[str, Any]]:
        """
        Retrieve task record by task ID (synchronous).
        ...
        Returns:
            Task record dict or None
        """
        query = "SELECT * FROM task_results WHERE task_id = %(task_id)s"
        result = self.db.execute(query, {"task_id": task_id}, fetch_one=True)
        return dict(result) if result else None
