...
class UserService:
    ...
    async def create_user(self, user_in: UserCreate) -> User:
        ...
        try:
            ...
            user = User(
                ...
                role="manager" if complete_user.is_superuser else "user",
    ...
    async def get_user_by_id(self, user_id: UUID) -> Optional[User]:
        ...
        try:
            ...
            return User(
                ...
                username=user_in_db.email.split("@")[0] if user_in_db.email else f"user_{str(user_in_db.id)[:8]}",
    ...
    async def update_user(self, user_id: UUID, user_update: UserUpdate) -> Optional[User]:
        ...
        try:
            ...
            return User(
                id=str(updated_user_in_db.id),
                username=updated_user_in_db.email.split("@")[0] if updated_user_in_db.email else f"user_{str(updated_user_in_db.id)[:8]}",
                email=updated_user_in_db.email,
                full_name=updated_user_in_db.full_name,
                role="manager" if updated_user_in_db.is_superuser else "user",
    ...
    async def list_users(
        ...
        try:
            ...
            return [
                User(
                    id=str(user.id),
                    username=user.username or (user.email.split('@')[0] if user.email else f"user_{str(user.id)[:8]}"),
                    email=user.email,
                    full_name=user.full_name,
                    role="manager" if user.is_superuser else "user",
    ...
    async def count_users(
        ...
        """Count users for bootstrap and manager-safety checks."""
        try:
    ...
    async def authenticate_user(self, username_or_email: str, password: str) -> Optional[UserInDB]:
        ...
        Includes 10-second overall timeout to prevent hanging on slow DB.
        ...
        try:
            ...
            return await asyncio.wait_for(_authenticate(), timeout=30.0)

        except asyncio.TimeoutError:
            logger.error(f"Authentication timeout for {username_or_email}")
            raise DatabaseConnectionError("Database unreachable: authentication timed out")
        except Exception as e:
