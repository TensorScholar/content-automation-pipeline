"""Decision Engine - Hierarchical Content Strategy Intelligence.

Implements a 3-layer decision hierarchy for content generation guidance:
1. Explicit Rules (Rulebook) - Highest authority, project-specific rules
2. Inferred Patterns - Medium authority, learned from historical data
3. Best Practices - Lowest authority, industry standards and guidelines

Decisions aggregate evidence from all layers with weighted confidence scoring
to provide actionable guidance with full audit trails.

Architecture:
    Evidence-based reasoning with semantic similarity matching across all layers.
"""
...
class Evidence:
    ...
    Represents information from any decision layer with semantic embeddings,
    confidence scoring, and authority weighting for aggregation.

    Attributes:
        source_layer: Decision layer this evidence originates from.
        content: Text content of the evidence.
        embedding: Semantic embedding vector for similarity matching.
        confidence: Evidence confidence score (0-1).
        authority: Layer-specific weight multiplier (0-1, 1=highest authority).
...
@dataclass
class Decision:
    """Final decision with complete provenance and confidence metrics.

    Aggregates evidence from multiple layers with weighted scoring to produce
    an auditable decision with full chain-of-reasoning.

    Attributes:
        query: Original decision query/question.
        choice: Selected decision or recommendation.
        confidence_score: Aggregate confidence (0-1).
        confidence_level: Categorical confidence rating.
        primary_layer: Layer contributing most authority to decision.
        evidence_chain: All evidence pieces that informed decision.
        reasoning: Human-readable explanation of decision logic.
        timestamp: Decision creation time (UTC).
...
class DecisionEngine:
    ...
    async def _gather_evidence(
        ...
        Args:
            ...
            rulebook_manager: Rulebook manager instance
        """
        evidence = []
    ...
    async def _query_explicit_rules(
        ...
        """
        try:
            rules = await rulebook_manager.query_rules(
                project_id=project_id,
                query=query,
                rule_type=rule_type,
                top_k=3,
