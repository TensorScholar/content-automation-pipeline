...
Security:
    All text inputs are sanitized using bleach to prevent:
    - ReDoS (Regular Expression Denial of Service) attacks
    - HTML/script injection
    - Oversized input DoS attacks (10k char limit)
"""
...
# Maximum allowed text length (10k chars) to prevent DoS
...
class SemanticAnalyzer:
    ...
    def __init__(self, redis_client: RedisClient, model_name: str = "all-MiniLM-L6-v2"):
        """
        Initialize semantic analyzer with embedding model configuration.
        ...
        self.embedding_dim = 384  # Default for all-MiniLM-L6-v2, updated when loaded

        logger.info(f"Semantic analyzer initialized (Lazy loading: {model_name})")

    @property
    def model(self):
        """Lazy load the SentenceTransformer model with automatic device detection."""
        if self._model is None:
            ...
            try:
                ...
                if torch.cuda.is_available():
                    ...
                    # Force CPU to prevent MPS crash on Apple Silicon during multiprocessing
                    # See: https://github.com/pytorch/pytorch/issues/88915
                    device = 'cpu'

                self._model = SentenceTransformer(self.model_name, device=device)
                self.embedding_dim = self._model.get_sentence_embedding_dimension()
                logger.success(f"Embedding model loaded: {self.model_name} ({self.embedding_dim}d) on {device.upper()}")
    ...
    def _sanitize_text(self, text: str) -> str:
        """
        Sanitize input text to prevent injection and DoS.
        ...
        # Clean text (strip tags, bleach defaults to safe tags only)
        # We strip all tags for semantic analysis as we want raw content
    ...
    async def _embed_single(
        ...
        if use_cache and self.cache_enabled:
            ...
            # Use fixed dim if not loaded yet, risk of mismatch if model changes but cache is persistent
        ...
        # Generate embedding (triggers model load)
    ...
    async def _embed_batch(
        ...
        if uncached_texts:
            ...
            for idx, embedding in zip(uncached_indices, embeddings):
                ...
                if use_cache and self.cache_enabled:
                    cache_key = self._compute_cache_key(cleaned_texts[idx], normalize)
