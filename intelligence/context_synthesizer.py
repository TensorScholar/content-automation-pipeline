"""Context Synthesizer - Intelligent LLM Context Compression.

Reduces token costs through multi-strategy semantic compression:

Strategies:
    1. Extractive Summarization: Sentence ranking via semantic centrality
    2. Semantic Deduplication: Merge similar/redundant information
    3. Information Filtering: Remove low-value content
    4. Hierarchical Abstraction: Adaptive detail levels
    5. Lazy Materialization: Load only required context

Target:
    70-85% token reduction with <5% information loss.

Architecture:
    Functional pipeline of composable compression transforms.

Example:
    >>> synthesizer = ContextSynthesizer(semantic_analyzer)
    >>> result = await synthesizer.compress(long_text, target_ratio=0.3)
    >>> print(f"Reduced tokens by {result.reduction_percent:.1f}%")
"""
...
class CompressionResult:
    """Complete compression operation result with metrics.

    Attributes:
        original_text: Input text before compression.
        ...
        information_density: Semantic preservation estimate (0-1, 1=perfect).
        method_chain: List of compression methods applied in order.
    """
...
class ContextPlan:
    ...
    Describes what context should be loaded and in what order without
    materializing it, enabling budget-constrained loading strategies.

    Attributes:
        essential_sources: Critical sources with relevance scores.
        optional_sources: Nice-to-have sources with relevance scores.
        compression_level: Target compression intensity.
        estimated_tokens: Expected token count after loading.
    """
...
class ContextSynthesizer:
    """Intelligent multi-strategy context compression engine.

    Implements composable compression pipelines:
    - Extractive summarization (sentence ranking by centrality)
    ...
    Compression is configurable by target ratio or token budget, with
    automatic strategy selection based on desired reduction level.
    """
    ...
    def __init__(self, semantic_analyzer: Optional[SemanticAnalyzer] = None):
        ...
        """
        try:
            # Lightweight spaCy pipeline for sentence tokenization.
            # Prefer en_core_web_sm when available, but gracefully degrade
            # to a blank English tokenizer so API routes remain functional.
            try:
                self.nlp = spacy.load("en_core_web_sm", disable=["ner", "parser"])
            except OSError:
                logger.warning(
                    "spaCy model 'en_core_web_sm' not available; falling back to blank English pipeline"
                )
    ...
    async def _extract_key_sentences(self, text: str, keep_ratio: float = 0.5) -> List[str]:
        ...
        # Use the instance's semantic analyzer
        analyzer = self.semantic_analyzer
        if analyzer is None:
            # Fallback: return sentences without ranking
            return sentences[:max(3, int(len(sentences) * keep_ratio))]

        # Generate embeddings
        embeddings = await analyzer.embed(sentences, normalize=True)

        # Compute document centroid
        centroid = analyzer.compute_centroid(embeddings, normalize=True)

        # Rank sentences by similarity to centroid
        scores = [
            analyzer.compute_similarity(emb, centroid, SimilarityMetric.COSINE)
    ...
    async def _deduplicate_sentences(self, sentences: List[str], threshold: float = 0.90) -> List[str]:
        ...
        keep_indices = analyzer.deduplicate_by_similarity(
    ...
    async def plan_context_loading(
        ...
        analyzer = self.semantic_analyzer
        if analyzer is None:
            # Fallback: return all as essential with minimal compression
            return ContextPlan(
                essential_sources=[(k, 1.0) for k in available_sources.keys()],
                optional_sources=[],
                compression_level=CompressionLevel.MINIMAL,
                estimated_tokens=sum(len(v) // 4 for v in available_sources.values()),
            )

        # Embed source descriptions
        descriptions = list(available_sources.values())
        desc_embeddings = await analyzer.embed(descriptions, normalize=True)

        # Rank by relevance to query
        relevance_scores = [
            analyzer.compute_similarity(query_embedding, emb, SimilarityMetric.COSINE)
    ...
    async def synthesize_context(
        ...
        """
        Synthesize context for content generation.

        This method gathers relevant context from project data, keywords,
        and optional instructions to create a coherent context object
        for LLM prompts.
        ...
        Returns:
            ProjectContext object with target_audience, tone, style_guide, custom_instructions
        """
        from core.models import ProjectContext

        # Extract keyword phrases for context
        keyword_phrases = [kw.phrase if hasattr(kw, 'phrase') else str(kw) for kw in keywords[:10]]

        # Build style guide from keywords and topic
        style_guide = f"Use clear, concise language. Support claims with data. Use active voice. Focus on: {', '.join(keyword_phrases[:5])}."

        context = ProjectContext(
            target_audience="General audience interested in technology and business",
            tone="Professional, authoritative, and informative",
            style_guide=style_guide,
            custom_instructions=custom_instructions,
        )

        logger.debug(f"Synthesized context for project {project_id}: {len(keyword_phrases)} keywords")
        return context
    ...
    async def _estimate_information_density(self, original: str, compressed: str) -> float:
        ...
        # Embed both texts
        orig_emb = await analyzer.embed(original, normalize=True)
        comp_emb = await analyzer.embed(compressed, normalize=True)

        # Cosine similarity = information preservation
        similarity = analyzer.compute_similarity(orig_emb, comp_emb, SimilarityMetric.COSINE)
...
def get_context_synthesizer() -> ContextSynthesizer:
    """Get or create the global ContextSynthesizer instance."""
