...
class LLMRequest(BaseModel):
    ...
    def validate_max_tokens(cls, v: int, info) -> int:
        ...
        if "gpt-4" in model and v > GPT4_MAX_TOKENS:
            raise ValueError(f"GPT-4 max tokens cannot exceed {GPT4_MAX_TOKENS}, got {v}")
        elif "gpt-3.5" in model and v > GPT35_MAX_TOKENS:
            raise ValueError(f"GPT-3.5 max tokens cannot exceed {GPT35_MAX_TOKENS}, got {v}")
        return v
...
class CircuitBreaker:
    ...
    def __init__(
        ...
        self._fallback_state = CircuitState.CLOSED
        self._fallback_failure_count = 0
        self._fallback_success_count = 0
        self._fallback_last_failure_time: Optional[datetime] = None
    ...
    async def call(self, func: Callable, *args, **kwargs):
        ...
        if current_state == CircuitState.OPEN:
            ...
            if (
                last_failure_time
                and (datetime.now(timezone.utc) - last_failure_time).total_seconds() > self.recovery_timeout
            ):
        ...
        try:
            ...
            await self._set_last_failure_time(datetime.now(timezone.utc))
    ...
    async def _get_state(self) -> CircuitState:
        """Get current circuit breaker state from Redis with in-memory fallback."""
        try:
            ...
            if not self._using_fallback:
                logger.warning(f"Redis unavailable for circuit breaker, using in-memory fallback: {e}")
                self._using_fallback = True
            return self._fallback_state

    async def _set_state(self, state: CircuitState, ttl: Optional[int] = None) -> None:
        """Set circuit breaker state in Redis with in-memory fallback."""
        try:
            if ttl:
            ...
            if not self._using_fallback:
                logger.warning(f"Redis unavailable, storing state in-memory: {e}")
                self._using_fallback = True
            self._fallback_state = state

    async def _get_failure_count(self) -> int:
        """Get current failure count from Redis with in-memory fallback."""
        try:
            count = await self.redis_client.get(self.failure_count_key)
            return int(count) if count is not None else 0
        except Exception:
            return self._fallback_failure_count if self._using_fallback else 0

    async def _increment_failure_count(self) -> int:
        """Atomically increment failure count in Redis with in-memory fallback."""
        try:
            count = await self.redis_client.increment(self.failure_count_key)
            return count
        except Exception:
            if self._using_fallback:
                self._fallback_failure_count += 1
                return self._fallback_failure_count
            return 0

    async def _reset_failure_count(self) -> None:
        """Reset failure count in Redis with in-memory fallback."""
        try:
    ...
    async def _get_success_count(self) -> int:
        """Get current success count from Redis with in-memory fallback."""
        try:
            count = await self.redis_client.get(self.success_count_key)
            return int(count) if count is not None else 0
        except Exception:
            return self._fallback_success_count if self._using_fallback else 0

    async def _increment_success_count(self) -> int:
        """Atomically increment success count in Redis with in-memory fallback."""
        try:
            count = await self.redis_client.increment(self.success_count_key)
            return count
        except Exception:
            if self._using_fallback:
                self._fallback_success_count += 1
                return self._fallback_success_count
            return 0

    async def _reset_success_count(self) -> None:
        """Reset success count in Redis with in-memory fallback."""
        try:
    ...
    async def _get_last_failure_time(self) -> Optional[datetime]:
        """Get last failure time from Redis with in-memory fallback."""
        try:
            ...
            return self._fallback_last_failure_time if self._using_fallback else None

    async def _set_last_failure_time(self, failure_time: datetime) -> None:
        """Set last failure time in Redis with in-memory fallback."""
        try:
            await self.redis_client.set(self.last_failure_time_key, failure_time.isoformat())
        except Exception:
            if self._using_fallback:
                self._fallback_last_failure_time = failure_time
...
class LLMClient(AbstractLLMClient):
    ...
    def __init__(
        ...
        # Per-minute rate limiting (token bucket algorithm)
        self._rate_limit_key = "llm_rate_limit"
        self._rate_limit_window = 60  # seconds
        self._max_requests_per_minute = (
            settings.llm.max_requests_per_minute if settings and hasattr(settings, "llm") else 50
        )
    ...
    async def _check_rate_limit(self) -> bool:
        ...
        try:
            ...
            key = f"{self._rate_limit_key}:{current_time // self._rate_limit_window}"
            ...
            if request_count >= self._max_requests_per_minute:
                logger.warning(
                    f"Rate limit exceeded: {request_count}/{self._max_requests_per_minute} requests/min"
                )
                return False
            ...
            await self.redis_client.expire(key, self._rate_limit_window * 2)
