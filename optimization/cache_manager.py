"""Cache Manager - Multi-Tier Caching Orchestration.

Implements 3-tier caching hierarchy:
    L1 (Memory): Hot data, microsecond access, LRU eviction
    L2 (Redis): Warm data, millisecond access, TTL-based
    L3 (Database): Cold data, 10-100ms access, read-through

Features:
    - Automatic cache warming on startup
    - Intelligent invalidation (dependency tracking)
    - Hit-rate monitoring and optimization
    - Circuit breaker for fault tolerance
    - Graceful degradation to lower tiers

Philosophy:
    Cache everything safely, invalidate intelligently.

Example:
    >>> cache = CacheManager(redis_client, max_memory_entries=1000)
...
@dataclass
class CacheEntry:
    """Single cache entry with access tracking and expiration.
...
class CachePolicy:
    """Cache eviction and invalidation policy definitions.

    Provides static methods for determining cache behavior:
...
class CacheManager:
    ...
    Thread-Safety:
        This class is designed for async concurrency. All mutating operations on
        the in-memory cache (L1) are protected by asyncio.Lock to prevent race
        conditions when accessed from multiple coroutines concurrently.

        - Memory cache operations use `self._memory_lock`
        - Redis operations are inherently atomic via Redis commands

    Note:
        The lock is created lazily on first access to avoid event loop issues
        when the CacheManager is instantiated before the event loop starts.
    """
    ...
    def __init__(
        ...
        # Thread-safety: Lock for memory cache operations
    ...
    def _get_memory_lock(self) -> asyncio.Lock:
        """
        Get or create the memory lock, ensuring it's bound to current event loop.

        This lazy initialization prevents 'attached to different loop' errors
        when CacheManager is instantiated before the event loop starts.
        """
    ...
    async def _get_from_memory(self, key: str) -> Optional[Any]:
        """Get value from L1 memory cache (thread-safe)."""
    ...
    async def _set_in_memory(
        ...
        """Set value in L1 memory cache (thread-safe)."""
        try:
            async with self._get_memory_lock():
    ...
    async def _evict_lru(self) -> None:
        """Evict least recently used entry from memory cache (thread-safe)."""
        async with self._get_memory_lock():
            await self._evict_lru_unlocked()

    async def _evict_lru_unlocked(self) -> None:
        """Evict LRU entry (must be called with lock held)."""
