...
class PromptTemplate:
    ...
    def _estimate_tokens(text: str) -> int:
        ...
        Uses 'cl100k_base' encoding (GPT-4/3.5-turbo standard).
        Falls back to character-based heuristic on failure.
        """
        if not text:
            return 0
...
class PromptCompressor:
    ...
    def _initialize_standard_templates(self) -> None:
        ...
        # Content Generation Template - Enhanced for WordPress Publishing
        ...
        content_template.add_component(
            PromptComponent(
                ...
                template="Generate a high-quality, SEO-optimized article on the topic: $topic",
                priority=10,
                required=True,
            )
        )
        ...
        content_template.add_component(
            PromptComponent(
                ...
                template="""Output as properly formatted HTML with WordPress compatibility. Target $word_count words.

CRITICAL FORMATTING RULES:
1. Use proper heading hierarchy: H2 for main sections, H3 for subsections, H4 for details
2. Use semantic HTML:
   - <ul> and <ol> for lists (REQUIRED for any enumeration)
   - <strong> for important keywords (use 2-3 per section)
   - <em> for emphasis on concepts
   - <blockquote> for notable quotes or key takeaways
3. Structure each section with:
   - Opening hook paragraph
   - Supporting details with lists where appropriate
   - Transitional conclusion
4. Include at least one <ul> or <ol> list in each major section
5. Do NOT output markdown - output clean HTML only
6. Do NOT include <!DOCTYPE>, <html>, <head>, or <body> tags - output ONLY the inner content HTML
7. Do NOT wrap output in code fences (```)
8. Paragraphs should be 2-4 sentences for readability""",
                priority=5,
                required=True,
            )
        )
