...
class TokenBudgetManager:
    ...
    def __init__(self, config: BudgetConfig):
        ...
        # Concurrency lock: serializes check-and-allocate to prevent budget overrun
        self._budget_lock = asyncio.Lock()

        logger.info(f"Token budget manager initialized: {config.daily_token_limit:,} tokens/day")

    async def set_daily_budget(self, tokens: int) -> None:
        """Adjust the daily token limit for testing scenarios."""
        self.config.daily_token_limit = tokens
        if BudgetPeriod.DAILY in self.usage:
            self.usage[BudgetPeriod.DAILY].tokens_limit = tokens
        # Update leaky bucket capacity/leak rate to reflect new limit
        self.rate_limiter.capacity = tokens
        self.rate_limiter.leak_rate = tokens / 86400
    ...
    async def request_budget(
        ...
        # Atomic check-and-allocate under lock to prevent budget overrun
    ...
    async def _process_pending_requests(self) -> None:
        ...
        for priority in sorted(Priority, key=lambda p: p.value, reverse=True):
            ...
            while queue:
                ...
                elapsed = (datetime.now(timezone.utc) - request.timestamp).total_seconds()
    ...
    def predict_exhaustion(self) -> Optional[datetime]:
        ...
        elapsed_hours = (datetime.now(timezone.utc) - daily.period_start).total_seconds() / 3600
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if exhaustion and exhaustion < datetime.now(timezone.utc) + timedelta(hours=6):
            recommendations.append(
