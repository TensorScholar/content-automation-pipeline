...
class ProjectRepository:
        ...
        try:
            async with self.database_manager.session() as session:
                ...
                if self.redis_client:
                    await self.redis_client.delete_pattern("query_cache:project_list:*")
    ...
    async def bulk_create(self, projects: List[Project]) -> List[Project]:
        """
        Bulk create multiple projects in a single database operation.

        PERFORMANCE FIX: Uses single INSERT with multiple VALUES instead of N individual INSERTs.
        This reduces N database round trips to just 1, fixing the N+1 query problem.

        Args:
            projects: List of Project models to create

        Returns:
            List of created Project models with IDs and timestamps
        ...
        try:
            async with self.database_manager.session() as session:
                ...
                if len(rows) != len(projects):
                    raise DatabaseError(
                        f"Bulk insert mismatch: expected {len(projects)} rows, got {len(rows)}"
                    )

                created_projects = [self._row_to_project(row) for row in rows]

                # Invalidate list cache
                if self.redis_client:
                    await self.redis_client.delete_pattern("query_cache:project_list:*")

                logger.info(f"Bulk created {len(created_projects)} projects in single query")
                return created_projects

        except Exception as e:
            logger.error(f"Failed to bulk create projects: {e}")
            raise DatabaseError(f"Bulk project creation failed: {e}")
    ...
    async def list_all(
    ...
    ) -> List[dict]:  # Return dicts since they are deserialized from cache
        """
        ...
        Returns:
            List of Project models (or dicts if from cache)
        """
        try:
            async with self.database_manager.session() as session:
            ...
            # Return as list of models so serializer can handle them
    ...
    async def update(self, project_id: UUID, updates: dict) -> Optional[Project]:
        ...
        try:
            ...
            async with self.database_manager.session() as session:
                ...
                updated_project = self._row_to_project(row)

                # Invalidate Caches
                if self.redis_client:
                    # Invalidate specific project cache
                    await self.redis_client.delete(f"query_cache:get_by_id:{str(project_id)}")
    ...
    async def update_last_active(self, project_id: UUID) -> bool:
        ...
        try:
            async with self.database_manager.session() as session:
                ...
                # Note: We don't invalidate cache for last_active updates to avoid cache thrashing
                # since this happens on every view
