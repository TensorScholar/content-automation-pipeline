...
class RulebookManager:
    ...
    async def create_rulebook(
        ...
        try:
            ...
            if not raw_content or not raw_content.strip():
                raise ValidationError("Rulebook content cannot be empty")
    ...
    async def _parse_and_index_rules(
        ...
        """
        Parse rulebook into semantic chunks and index as rules using BATCH processing.

        Optimized Strategy:
        1. Chunk content
        2. Batch generate embeddings (1 call)
        3. Bulk insert to DB (1 call)
        4. Bulk cache in Redis
        ...
        # Step 2: Batch Generate Embeddings (with graceful fallback)
        embeddings = None
        try:
            if self.embedding_model:
                embeddings = await self.embedding_model.embed(chunks)

                # Ensure embeddings is a list of lists/arrays
                if hasattr(embeddings, "tolist"): # It's a numpy array of arrays
                    embeddings = embeddings.tolist()
                elif isinstance(embeddings, list) and len(embeddings) > 0 and hasattr(embeddings[0], "tolist"):
                    embeddings = [e.tolist() for e in embeddings]
        except Exception as e:
            logger.warning(f"Embedding generation failed (continuing without embeddings): {e}")
            embeddings = None
        ...
        for idx, chunk in enumerate(chunks):
            # Metadata extraction (CPU bound, fast)
            rule_type = self._classify_rule_type(chunk)
            priority = self._extract_priority(chunk)
            context = self._extract_context(chunk)

            # Get corresponding embedding (None if not available)
            embedding_list = embeddings[idx] if embeddings and idx < len(embeddings) else None
            ...
            if embedding_list:
                redis_cache_ops.append((rule_id, embedding_list))

        # Step 4: Bulk Database Insert
        if rules_data:
            stmt = insert(rules_table).values(rules_data).returning(rules_table)
            ...
            # Step 6: Async Cache Population (with error handling)
            for rid, emb in redis_cache_ops:
                try:
                    ...
                    logger.warning(f"Redis cache failed for rule {rid}: {e}")

            logger.info(f"Batch indexed {len(rules)} rules with embeddings")
            return rules
