...
class UserRepository:
    ...
    async def get_by_id(self, user_id: UUID) -> Optional[UserInDB]:
        """
        Retrieve user by ID with timeout protection.
        ...
        try:
            ...
            return await asyncio.wait_for(_query(), timeout=15.0)


        except asyncio.TimeoutError:
            logger.error(f"Database timeout for user lookup by ID: {user_id}")
            from core.exceptions import DatabaseConnectionError
            raise DatabaseConnectionError("Database unreachable: timed out looking up user by ID")
    ...
    async def get_by_username_prefix(self, username_prefix: str) -> Optional[UserInDB]:
        """
        Retrieve user by username prefix (email prefix) with timeout protection.
        ...
        try:
            ...
            logger.error(f"Database timeout for user lookup by prefix: {username_prefix}")
            from core.exceptions import DatabaseConnectionError
            raise DatabaseConnectionError("Database unreachable: timed out looking up user by username")
        except Exception as e:
    ...
    async def get_by_email(self, email: str) -> Optional[UserInDB]:
        """
        Retrieve user by email address with timeout protection.
        ...
        try:
            ...
            logger.error(f"Database timeout for user lookup by email: {email}")
            from core.exceptions import DatabaseConnectionError
            raise DatabaseConnectionError("Database unreachable: timed out looking up user by email")
        except Exception as e:
    ...
    async def create(self, user_in: UserCreate, hashed_password: str) -> UserInDB:
        ...
        try:
            ...
            async with self.database_manager.session() as session:
                # Prepare user data - use UTC naive datetimes for PostgreSQL TIMESTAMP WITHOUT TIME ZONE
                now = datetime.now(timezone.utc).replace(tzinfo=None)
                user_data = {
                    "email": user_in.email.lower().strip(),
                    "hashed_password": hashed_password,
                    "full_name": user_in.full_name,
                    "is_active": True,  # Allow immediate login - permanent fix
                    "is_superuser": False,
                    "created_at": now,
                    "updated_at": now,
                }
        ...
        try:
            async with self.database_manager.session() as session:
                query = (
                    update(users_table)
                    .where(users_table.c.id == user_id)
                    .values(hashed_password=hashed_password, updated_at=datetime.now(timezone.utc).replace(tzinfo=None))
                )
                result = await session.execute(query)
                await session.commit()
