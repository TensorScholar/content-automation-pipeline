...
Type-safe exception taxonomy with:
- Structured context propagation (error IDs, severity, metadata)
- Retry metadata for automatic recovery
- Exception chaining for causal analysis
- Observability integration (logging, metrics)

Architecture:
    Railway-oriented programming with explicit error types.
    All exceptions inherit from ContentAutomationException.
...
class ContentAutomationException(Exception):
    ...
    - error_id: Unique UUID for distributed tracing
    - severity: ErrorSeverity enum for alerting thresholds
    - context: Dict with error-specific metadata
    - error_code: Optional machine-readable error identifier
    - retryable: Boolean indicating if operation can be retried
    - timestamp: UTC timestamp for temporal analysis
...
class LLMConnectionError(LLMException):
    """Cannot reach LLM provider (network blocked, no internet, DNS failure)."""

    def __init__(
        self,
        message: str = "Cannot connect to LLM provider. Check network access.",
        **kwargs,
    ):
        super().__init__(
            message,
...
class InfrastructureException(ContentAutomationException):
    ...
    def __init__(self, message: str, **kwargs):
        # Only set default severity if not provided by subclass
