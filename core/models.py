"""Domain Data Models - Type-Safe Pydantic Schemas.

Complete Pydantic v2 model definitions with:
- Automatic validation and type coercion
- Computed properties and derived fields
- Optimized JSON serialization
- Immutability enforcement where appropriate

Architecture:
    Domain-Driven Design with value objects and entity patterns.
    All models inherit from BaseModelConfig for consistent behavior.
...
class BaseModelConfig(BaseModel):
    """Base configuration inherited by all domain models.

    Provides:
        - Validation on field assignment
        - Enum preservation (no auto-conversion to strings)
        - Support for numpy arrays and custom types
        - Custom JSON encoders for datetime, UUID, numpy
    """
...
class Project(BaseModelConfig):
    ...
    vertical: Optional[str] = Field(
        default=None,
        description="Industry vertical (e.g., 'B2B SaaS', 'E-commerce', 'Healthcare')",
    )
    ...
    target_audience: Optional[str] = Field(
        default=None,
        description="Target audience description (e.g., 'Technical Decision Makers')",
    )
...
class ValidationIssue(BaseModelConfig):
    """Single validation issue found during content validation."""

    severity: str = Field(..., pattern="^(error|warning|info)$")
    category: str = Field(..., description="Category of the issue (e.g., 'word_count', 'keywords', 'structure')")
    message: str = Field(..., description="Human-readable description of the issue")
    expected: Optional[str] = Field(default=None, description="Expected value or requirement")
    actual: Optional[str] = Field(default=None, description="Actual value found in content")
    suggestion: Optional[str] = Field(default=None, description="Suggested fix or action")


class ValidationDetails(BaseModelConfig):
    """
    Detailed validation results comparing generated content against requirements.

    Stores specific validation findings to help identify quality issues.
    """
    ...
    word_count_met: bool = Field(default=True, description="Whether target word count requirement was met")
    word_count_deviation_pct: float = Field(default=0.0, description="Percentage deviation from target word count")

    keywords_met: bool = Field(default=True, description="Whether all primary keywords are present")
    missing_keywords: list[str] = Field(default_factory=list, description="List of required keywords not found")

    structure_met: bool = Field(default=True, description="Whether required sections are present")
    missing_sections: list[str] = Field(default_factory=list, description="List of required sections not found")
...
class GeneratedArticle(BaseModelConfig):
    ...
    validation_result: ValidationResult = Field(default=ValidationResult.PASS)
    validation_details: Optional[ValidationDetails] = Field(default=None, description="Detailed validation results")
...
class UserCreate(BaseModelConfig):
    ...
    def validate_password(cls, v: str) -> str:
        ...
        # Require special character for production security
